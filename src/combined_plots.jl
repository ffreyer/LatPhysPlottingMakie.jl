function BandstructureGUI(
    ...
    for (i, label) in enumerate(labels)
        s = sliders[2 + 2div(i-1, 4), mod1(i, 4)] = Slider(
            fig, range = slider_range, width = Relative(1)
        )
        sliders[1 + 2div(i-1, 4), mod1(i, 4)] = Label(
            fig,
    ...
    path_signal = Observable(path)
    bs = Observable{AbstractBandstructure}(getBandstructure(uc, path_signal[], H))
